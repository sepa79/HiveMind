...
docker push registry.company.example/hivemind-api:0.1.0
```
...
```bash
code --add-mcp '{"name":"hivemind","type":"stdio","command":"npx","args":["-y","/path/to/hivemind-mcp-0.1.0.tgz"],"env":{"HIVEMIND_API_BASE_URL":"https://hivemind.company.example"}}'
```
