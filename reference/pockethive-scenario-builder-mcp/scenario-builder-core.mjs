...
export function validateWizardSession(session) {
  ...
  if (!Array.isArray(bees) || bees.length === 0) {
    issues.push(validationIssue("BEES_REQUIRED", "error", "template.bees", "Scenario must define at least one bee."));
  }
  ...
  for (const [index, bee] of (bees ?? []).entries()) {
    ...
    if (beeIndex.has(bee.id)) {
      issues.push(validationIssue("BEE_ID_DUPLICATE", "error", `${path}.id`, `Bee id '${bee.id}' must be unique.`));
    }
...
function applyRequestBuilderConfigStep(session, input) {
  ...
  session.summary.push(`request-builder config.templateRoot set to '${input.requestBuilder.config.templateRoot}'.`);
}
...
function applyTemplateHttpPutStep(session, input) {
  ...
  session.summary.push(`HTTP template '${input.template.callId}' configured for service '${input.template.serviceId}'.`);
}
